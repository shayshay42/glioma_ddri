...
include("../scripts/setup/compute_dose_3.jl")
...
function generate_population(drug, num_patients, seed)
    ...
    return population, drug_params
end

function compute_doses(population, drug_params, drug, method)
    ...
    elseif method == "optim2"
        ic50_fractional_doses, minimal_objectives = compute_dose3(population, drug_params)
    else
    ...
    # Package results in a NamedTuple
    results = (doses = ic50_fractional_doses, residuals = residuals)
    if method == "optim1" || method == "optim2"
...
# Generate population
population, drug_params = generate_population(drug, num_patients, seed)

# Compute doses
dose_results = compute_doses(population, drug_params, drug, method)
dose_results.objectives
#
...
method = "optim1"
dose_results_bfgs = compute_doses(population, drug_params, drug, method)

method = "bvp"
dose_results_bvp = compute_doses(population, drug_params, drug, method)

method = "optim1" #changes the inner compute_dose to NelderMead
dose_results_nelder = compute_doses(population, drug_params, drug, method)

#MISSING METHOD FOR STOCHASTIC GRADEINT DESCENT TO SEE IF WE CAN ESCAPE LOCAL MINIMA
...
function simulate_to_plot(amount_per_mult_per_patient, population, drug, pk_params, mults=[0.01, 0.1, 0.5, 1.0, 1.5, 2.0])
    ...
    prob = ODEProblem(drug_pk!, u0, tspan, p)
    # tspan = (0.0, 18.0).*24.0
    function output(dose, pk_values)
        tmp_prob = remake(prob, p=[pk_values...,dose])
        tmp_sol = solve(tmp_prob, callback=cb, saveat=0.1)# abstol=1e-10, reltol=1e-10,dtmax=1)#, alg_hints=[:stiff])
        # peak_PlaRG = maximum(tmp_sol[2, :])  # Assuming PlaRG is the second variable
    ...
    pk_indices = indexin(pk_param_names, param_order)
    pk_param_values= population[pk_indices, :]
    sol = output(amount_per_mult_per_patient[1,1], pk_param_values[:,1])
    timepoints = sol.t
    ...
    for (j,mult) in enumerate(mults)
        for (i,dose) in enumerate(amount_per_mult_per_patient[j,:])
            ...
            # input_dose = Optim.minimizer(result)
            # patient_doses[j, i] = input_dose[1]
            # println("$(pk_param_values[:,i])")
            sol = output(dose, pk_param_values[:,i])
            sols[j,i,:] = sol[2,:]
            # end
        end
    end
...
using Plots
names = ["BFGS", "BVP", "NelderMead"]
for (j,dose_matrix) in enumerate([dose_results_bfgs.doses, dose_results_bvp.doses, dose_results_nelder.doses])
    times, sols = simulate_to_plot(dose_matrix, population, "rg", drug_params)
    for (i,mult) in enumerate([0.01, 0.1, 0.5, 1.0, 1.5, 2.0])
        data_matrix = sols[i,:,:]
        timepoints = times ./24

        title = "VP Trajectories - RG model Plasma - $(mult) IC50 - $(names[j])"
        means = vec(mean(data_matrix, dims=1))

        # Plot the trajectories with a label for the legend
        plot(timepoints, data_matrix'[:,1]     , alpha=0.3, linecolor=:lightgray, grid=false, label="Plasma Amount", legend=:topleft)
        ...
        savefig("$(drug)_model_$(mult)_IC50_spaghetti_$(names[j]).png")
    end
end
...
# compare_matrices(dose_results, bvp_results)
