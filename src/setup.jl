...
struct Patient
    ...
    scaling::Tuple{Float64, Float64, Float64, Float64}
...
function generate_patients_struct(num_patients, seed, drug)
    ...
    for i in 1:num_patients
        ...
        for (j, effect) in enumerate(effect_keys)
            patients[i].output_measures[effect] = get_outputs(population[:,i], ones(length(drug_dose_times)).*avg_dose_per_gradation[j], scaling[i], drug)
        end
