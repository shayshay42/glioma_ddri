    ...
    Threads.@threads for i in 1:size(population, 2)
        ...
        for (j,mult) in enumerate(gradation)
            function bc_drug_pk!(residual, sol, p, t)
                ...
                residual[1] = sol[1][1] # Ensure that the initial condition is fixed to 0
                residual[2] = sol[1][2]
                residual[3] = sol[1][3]
                residual[4] = maximum(E_sol) - mult # Ensure the plasma compartment matches ic50 amount conversion at end time
            end
            u0_temp = [17.7, 0.0, 0.0, 0.0, 1000.0] #1000.0 as an initial guess for the dose
            prob_temp = BVProblem(drug_pk_dose!, bc_drug_pk!, u0_temp, tspan, ode_p)
            try
                sol_temp = solve(prob_temp, Shooting(Rodas4P2()), callback=cb)
                input_dose = sol_temp[1][end]
