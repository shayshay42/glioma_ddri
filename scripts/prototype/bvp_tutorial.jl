...
# Define your θ_range and dθ_range as before
θ_range = range(-2π, stop=2π, length=40)
dθ_range = range(-10, stop=10, length=40)
...
# Prepare data for the arrows
positions = [(θ, dθ) for θ in θ_range, dθ in dθ_range]
directions = [let du = zeros(Float64, 2)
                  simplependulum!(du, [θ, dθ], [], 0.0)
                  du
              end for (θ, dθ) in positions]

# Flatten the arrays for compatibility
flat_positions = reduce(vcat, positions)
flat_directions = reduce(vcat, directions)

# Create the plot using arrows!
arrows(scene, flat_positions, flat_directions, color=:gray)
